...
function text(root: ReturnType<typeof create>) { return root.root.findAllByType(Text).map(node => Array.isArray(node.props.children) ? node.props.children.join('') : String(node.props.children)).join(' '); }
...
beforeEach(() => { jest.clearAllMocks(); mockStorage.read.mockResolvedValue(null); mockSend.mockResolvedValue(success); });
it('shows authoritative context, optional card/transfer reconciliation and requires confirmation', async () => {
  ...
  act(() => input(root, 'Efectivo contado', '1020.00'));
...
it('denomination helper uses backend values and displays shortage/surplus', async () => {
  let root!: ReturnType<typeof create>; await act(async () => { root = create(React.createElement(CloseScreen)); });
  act(() => press(root, 'Denominaciones'));
  act(() => input(root, 'Cantidad de 500', '2'));
  expect(text(root)).toContain('Faltante');
  act(() => input(root, 'Cantidad de 20', '2'));
  expect(text(root)).toContain('Sobrante');
  act(() => press(root, 'Revisar cierre'));
  await act(async () => { press(root, 'Cerrar caja'); });
  expect(mockSend.mock.calls[0][0].request.counted_cash).toBe('1040.00');
  expect(mockSend.mock.calls[0][0].request.counted_denominations).toEqual({ '500': 2, '20': 2 });
  act(() => root.unmount());
});
it('restores an uncertain close without sending and Retry uses the saved payload', async () => {
  const request = { operation_uuid: success.operationUuid, register_id: 6, counted_cash: '1020.00', notes: 'Frozen' };
...
it('401 signs out and preserves close attempt', async () => {
  ...
  let root!: ReturnType<typeof create>; await act(async () => { root = create(React.createElement(CloseScreen)); });
  act(() => input(root, 'Efectivo contado', '1020.00')); act(() => press(root, 'Revisar cierre'));
  await act(async () => { press(root, 'Cerrar caja'); });
...
it('business rejection offers correction and does not navigate as success', async () => {
  ...
  expect(text(root)).toContain('Revisión de caja'); act(() => root.unmount());
});
