...
export function CartSummaryBar({ itemCount, totalCents, onViewCart, onCheckout }: Props) {
  ...
  return (
    <View style={[styles.wrapper, { bottom: insets.bottom + 56 }]}>
      <View style={styles.info}><Text style={styles.count}>{hasItems ? `${itemCount} ${itemCount === 1 ? 'artículo' : 'artículos'}` : 'Carrito vacío'}</Text><Text style={styles.total}>{formatMinorUnits(totalCents)}</Text></View>
...
const styles = StyleSheet.create({
  wrapper: { position: 'absolute', left: spacing.md, right: spacing.md, minHeight: 72, flexDirection: 'row', alignItems: 'center', gap: spacing.sm, padding: spacing.sm, borderRadius: radii.md, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.line, zIndex: 10 },
