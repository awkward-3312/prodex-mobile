...
export function CartItemRow({ item, onIncrease, onDecrease, onRemove }: Props) {
  return (
    <View style={styles.row}>
      ...
      <View style={styles.total}><Text style={styles.subtotal}>{formatMinorUnits(calculateLineSubtotalMinorUnits(item.unitPriceCents, item.quantity))}</Text><Pressable accessibilityLabel={`Eliminar ${item.product.name}`} accessibilityRole="button" onPress={onRemove} style={styles.remove}><Ionicons name="trash-outline" size={17} color={colors.red} /></Pressable></View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  variant: { marginTop: 2, color: colors.inkMuted, fontSize: 11, fontWeight: '700' },
