...
export function ProductCard({ product, onPress }: Props) {
  const isUnavailable = product.stockStatus === 'Sin stock';
  ...
  return (
    <Pressable accessibilityLabel={`${product.name}, ${formatCurrency(product.price)}, ${product.stockStatus}`} accessibilityRole="button" accessibilityState={{ disabled: isUnavailable }} disabled={isUnavailable} onPress={onPress} style={({ pressed }) => [styles.card, isUnavailable && styles.disabled, pressed && styles.pressed]}>
      <View style={[styles.image, { backgroundColor: tone.background }]}><Ionicons name={product.icon} size={28} color={tone.icon} /></View>
      ...
      <View style={styles.stockRow}>
        ...
        <Text style={[styles.stock, product.stockStatus === 'Bajo stock' && styles.lowStock, isUnavailable && styles.unavailable]}>{isUnavailable ? 'Sin stock' : `${product.stock} disponibles`}</Text>
      </View>
    </Pressable>
  );
}
...
const styles = StyleSheet.create({
  ...
  image: { height: 90, borderRadius: radii.sm, alignItems: 'center', justifyContent: 'center' },
